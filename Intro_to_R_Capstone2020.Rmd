...
RStudio is a graphical user interface (GUI) that makes it much easier to interact with the R software. It adds handy drop-down menus, visualizations, and other features that make your life easier. R does the work, RStudio makes it pretty. 


### Why R?

1. **It's Free!:** Obviosuly a benefit since other statistical programs require subscriptions in the range of thousands of dollars. It's also open-source which means anyone can inspect is and report bugs, reducing the chance of mistakes. 
...
Personalizing your RSudio experience:
...
R Scripts are also quite useful for saving code. These files are a bit more "simple" than R markdown files in that they contain only code and any notes/comments must be "commented out" using a # symbol. Another difference is where the result appears. When using an R script, figures generally appear in the plots pane of R studio and other calculations appear in the consol. For code for you actual analyses, I would recommend a R Script over a markdown file, but it's really a personal preference. 
...
If you ever need to change your working directory (analogous to walking down the hall to your neighbor's office), you can use the below code. If you run this code as is, you will get an error message because the file path I wrote does not exist on your computer (I hope...).
 
#### *Challenge #x:*
##### *Change the file path below to the path to your Capstone directory or other folder that actually exists on your computer*
...
When you start doing more complex calculations that use much more computer power, you won't want to run the function everytime you need it. Instead, you can store the values and name them using the "assignment operator" (<-). Think of this as an arrow assigning anything on the right to the name on the left. Below is an example. Notice that when you run this code, "x" appears in your global environment (the upper right panel in my R Studio).
...
#### *Challenge #x:*
##### *Can you create an object named "y" that is 3 times the value of x?*
...
The above function only takes one arguement (the number you want to take the square root of) but many functions take multiple arguements. Another simple example is the round() function which takes two arguements: the number you want to round (x), and the number of resulting decimal places you would like (digits). These can be defined. (Also notice that pi is pre-defined in R).
...
Packages are basically a collection of functions that aren't included in base R, which allows us to access a lot more functionalility of R without having to write all of the code from scratch ourselves. To access a package you need to do two things: intall it and load it. You only need to install a package once on your computer (unless you need to re-download or update R) but you need to load the package each time you open R and wish to use it. Run the code below to install the "lubridate" package. We will use this later in the tutorial.
...
I've said it before but I will say it again: you do not need to memorize any of the functions in R, you just need to know where to find the necessary information about them. One easy way to to this is the help page in R. If you remember the name of a function but don't remember what it does or the arguements it takes, or see a function you are not familiar with, simply type the name of the function with a question mark in front of it to access the help page for the function. Run the code below to learn about the "plot()" function in R (one that we will use quite a bit...). What arguements does this function require?
...
Vectors can also be charater values (aka letters, words, or a combination of letters and numbers). Note that the quotes are necessary, it tells R to treat these words as something called character strings rather than looking for an object named "blackberries" that does not exist.
...
#now we combine them into a data fram
...
You can (thankfully) also specify columns by name:
```{r}
data$names
```

We can also subest using logical statements (TRUE or FALSE statements). Lets start by seeing if anyone has a height greater than 70 inches.
...
Download this spreadsheet and save it in the same folder as this R markdown/R project (which should be your working directory). If you need to, feel free to double check where your working directory is as this is a common source of error messages.

```{r}
covid_data <- read.csv("~/Documents/College_Crap/Research/Data Analysis Resources/CapstoneStuff/Capstone2020/countries_aggregated8.21.20.csv")
```

First let's look at the structure of our data frame. What type of data is present in each column? This is important because often data doesn't import the exact was we might want it to. Pay special attention to the "Date" column. 
...
#### *Challenge #x:*
##### *Go back to the help page for the plot() function. What does type="l" do? Look at some of the other arguements I added too.*

&nbsp

Lets add some more data to our plot. Once we have an established plot, we can add more data to it using the points() and lines() functions. These take most of the same arguements as the plot() function. 
...
#adding a legend
legend ("topleft", legend=c("Confirmed", "Deaths"), col=c("darkblue", "red"), lty=1)
```
...
Up until now we have been using base R to plot. There is a lot we can do with R's built-in plotting functions, but it is somewhat limitted. This is why I often use the ggplot2 package for most of the figures I make. The coding is a bit more confusing at first but it's well-worth it for the pretty graphs!! In case you need a reminder: here's my favorite source of amazing ggplot (and base R) graphics ideas and code: https://www.r-graph-gallery.com/ggplot2-package.html
...
You'll notice that this only shows a blank plot (no data). From here we can add components (data points, lines, themes, labels, etc.): I would encourage you to run each line below individually and view your graph each time to see how it changes :) 

```{r}
us_plot2 <- us_plot2 + geom_line (col="blue")
us_plot2 <- us_plot2 + theme_classic()
us_plot2 <- us_plot2 + labs (x="Confirmed Cases", title="COVID cases over time in the US")
us_plot2
```
...
top_countries <- covid_data[covid_data$Date=="2020-08-18" & covidA$Confirmed>500000,]$Country
...
##### *Can you add do this data a column diplaying fatality rates?*
...
Congratualions! You have completed the tutorial. Go up to the top of this panel in RStudio and click preview. This will give you a preview of you completed R tutorial and resource guide. You will also notice a file with the same name as this R markdown in the same folder with a .html extension. You can open this (it will open in an internet brower) and use it as a nice source of code and information whenever you need. Good luck on you future coding!
